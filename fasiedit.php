...
//query database
	$query = "SELECT * FROM fasilitator WHERE idFasi = '$idfasi'";
...
//arahan keluarkan satu per satu result
	while($row=mysqli_fetch_assoc($result)){
		...
		$id = $row['idFasi'];
		$nama = $row['fnama'];
		$nomatrik = $row['fnoMatrik'];
		$noic = $row['noic'];
		$jabatan = $row['fjabatan'];
		$sem = $row['semester'];
		$notel = $row['fnoTel'];
		$kelas = $row['fkelas'];
...
<form action="fasiedit2.php" method="POST">	
		...
		<input type="text"  name="nomatrik" maxlength="12" required value="<?php echo "$nomatrik"; ?>">
	 			
         <label>No Ic</label>
		<input type="text"  name="nokp"  required value="<?php echo "$noic"; ?>">
		...
		<input type="text" name="semester" required value="<?php echo "$sem"; ?>">
		
		<label>No Telefon</label>
		<input type="text" name="notel" required value="<?php echo "$notel"; ?>">
		
		<label>Kelas</label>
		<input type="text" name="kelas" required value="<?php echo "$kelas"; ?>">
		
        <input type="hidden" name="idf" value="<?php echo "$id"; ?>">
