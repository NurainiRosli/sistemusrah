<?php
	...
	$nottaken=0;
	$ttaken=0;
	$strno=$_POST['nomatrik'];
	
	// Student data collection
	$sql = "SELECT idstudent,namepelajar,nomatrik FROM student where $strno=nomatrik";
...
<?php if (count($result)) : ?>
		...
		<?php
		    $tempid=$result[0]['idstudent'];
			$tempnm=$result[0]['namepelajar'];
			$rollno=$result[0]['nomatrik'];
		?>
		...
		<div class="container">
			...
			<div class="col-md-12 col-lg-12">
			<?php
				if ($_POST['student'] === 'y' && isset($_POST['nomatrik'])) {
				...
				echo "<table class='table table-striped table-hover reports-table'>";
					 ...
					 $ssql = "SELECT idclass FROM student_class where $tempid=idstudent";
				 ...
				 $stmt3->execute();
					...
					$dttaken=0;
						echo"<tr>";
						$subid=$result3[$nosub]['idclass'];
						$sqql = "SELECT nameclass FROM class where $subid=idclass";
						 $stmt4 = $conn->prepare($sqql);
					$stmt4->execute();
						$result4 = $stmt4->fetchAll(PDO::FETCH_ASSOC);	
						$sub=$result4[0]['nameclass'];
						echo "<td><h6>$sub</h6></td>";
						for($i=0;$i<count($result2);$i++)
						 {
							$tmdat=$result2[$i]['date'];
							$sql1= "SELECT ispresent FROM attendance where idstudent=$tempid AND idclass=$subid AND date=$tmdat ORDER BY date";
                ...
                <div class="panel-body">
                  <p>Present Days out of Working Days:&nbsp;<strong>' . $present . '/' . $tlec . '</strong></p>
					...
					header("location:studentdata.php?student=invalid");
				}
...
<?php else: ?>
	<?php header("location:studentdata.php?student=invalid"); ?>
<?php endif; ?>